...
function getpaymentedit(id) {
    $.ajax({
        url: base_url + 'fees/paymentDetails',
        type: 'POST',
        data: { 'id': id },
        dataType: "json",
        success: function(data) {
            ...
            $('#fineAmount').val(data.fine);
            $('#payment_method').val(data.pay_via).trigger('change');
            $('#date').val(data.date);
            $('#remarks').val(date.remarks);
            $("#editfees_type").append(new Option(data.feetype_name, data.feetype_id));
            $('#editfees_type').val(data.feetype_id).trigger('change');
