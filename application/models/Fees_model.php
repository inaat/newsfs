...
class Fees_model extends MY_Model
{
    ...
    {  
        ...
        fee_groups_details ON fee_groups_details.fee_groups_id = fee_allocation.group_id 
        LEFT JOIN fees_type ON fees_type.id = fee_groups_details.fee_type_id WHERE
    ...
    {
        $this->db->select('s.id,e.branch_id,p.father_name,s.first_name,s.mobileno as stdmobile,s.last_name,s.photo,e.roll,s.email as student_email,s.current_address as student_address,c.name as class_name,b.school_name,b.email as school_email,b.mobileno as school_mobileno,b.address as school_address');
    ...
    {
        $this->db->select('fa.session_id,e.student_id,e.roll,s.first_name,s.last_name,s.register_no,s.mobileno,c.name as class_name,se.name as section_name');
    ...
    public function getDueStudentReport($class_id='', $section_id='',$session_id='')
    {
        $this->db->select('fa.id as allocation_id,sum(gd.amount) as total_fees,e.student_id,e.roll,s.first_name,s.last_name,s.register_no,s.mobileno,c.name as class_name,se.name as section_name');
        $this->db->from('fee_allocation as fa');
        $this->db->join('fee_groups_details as gd', 'gd.fee_groups_id = fa.group_id', 'left');
    ...
    {
        $this->db->select('h.*,t.name, t.name as feetype_name, t.id as feetype_id ,pt.name as payvia');
        $this->db->from('fee_payment_history as h');
        $this->db->join('fees_type as t', 't.id = h.type_id', 'left');
        $this->db->join('payment_types as pt', 'pt.id = h.pay_via', 'left');
